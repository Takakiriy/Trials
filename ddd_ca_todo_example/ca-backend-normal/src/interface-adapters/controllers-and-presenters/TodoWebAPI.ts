...
export class TodoPresentation {
    ...
    constructor(private readonly useCase: TodoInputPort) {
        this.router = Router();
        this.register();
    }
...
class ExpressTodoOutputPort implements TodoOutputPort {
    ...
    presentMany(todos: Todo[]): void {
        this.result = { status: 200, body: { success: true, data: todos.map(toJson) } };
    }
