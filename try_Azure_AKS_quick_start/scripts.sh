...
export  g_ResourceGroupName="sage-p-my-test-kube-12345"

export  g_ClusterName="MyTestKube"

export  g_Region="southeastasia"

export  g_KubernatesYamlFileName="azure-vote.yaml"
#//==================================================================
...
echo "Skipped"  ;fi
	...
	Install_AzureAKS_func
	Deploy_AzureAKSQuickStart_func
}
...
function  Uninstall_func()
{
	...
	#// Uninstall
	UninstallWithConfirm_AzureAKS_func
...
#********************************************************************
# Function: Deploy_AzureAKSQuickStart_func
#********************************************************************
function  Deploy_AzureAKSQuickStart_func()
{
	EchoNextCommand_func

	kubectl apply -f "${g_KubernatesYamlFileName}"


	echo  ""
	ColorEcho_func  "azure-vote-front の EXTERNAL-IP が設定されたら、30秒後にブラウザーでそのIPアドレスを開いてください。\n" \
		"Green"
	echo  "スクリプトの実行は以上です。 Ctrl+C で終了します。"
	EchoNextCommand_func
...
#********************************************************************
# Function: SetUpVariables_AzureAKS_func
#********************************************************************
function  SetUpVariables_AzureAKS_func()
{
	export  g_Kubectl_PATH="${HOME}/.azure-kubectl"
	export  g_Kubelogin_PATH="${HOME}/.azure-kubelogin"
	export  g_AzureAKS_DependenciesTitle="AzureAKS"
	export  g_AzureAKS_Dependencies=( "AzureAKS" "AzureCLI" "Node.js" )

	export  PATH="$PATH:${g_Kubectl_PATH}:${g_Kubelogin_PATH}"  #// kubectl command
}


#********************************************************************
# Function: Install_AzureAKS_func
#********************************************************************
function  Install_AzureAKS_func()
{
	if [ ! -e "${g_Kubectl_PATH}" ]; then

		echo  ""
		ColorEcho_func  "Azure AKS のクラスターのデプロイと kubectl のインストール\n"  "Green"
		ColorEcho_func  "Azure にログインする。ブラウザーが開くのでアカウントを選んでください\n"  "Green"
		if [ "$(az account list 2> /dev/null)" == "[]" ]; then
			EchoNextCommand_func

			az login
		else
			ColorEcho_func  ""  "White"  #// az account list で [] を返すと色が変わったままになるため
			echo  "Skipped. Because already logged-in."
		fi
		...
		if [ "${group_exists}" == "true" ]; then
			Error_func  "リソース グループ ${g_ResourceGroupName} は既に存在しています。削除中ならしばらく待ってから再実行してください。"
		fi


		echo  ""
		ColorEcho_func  "Kubenetes クラスター関連のリソース グループを作成する\n"  "Green"
		EchoNextCommand_func

		az group create  --name "${g_ResourceGroupName}"  --location "${g_Region}"


		echo  ""
		ColorEcho_func  "Kubenetes クラスターを作成する\n"  "Green"
		EchoNextCommand_func

		az aks create  --resource-group "${g_ResourceGroupName}"  --name "${g_ClusterName}" \
			--node-count 1  --enable-addons monitoring  --generate-ssh-keys


		echo  ""
		ColorEcho_func  "kubectl をインストールする。\n"  "Green"
		echo  "\"${g_Kubectl_PATH}/kubectl.exe\" が作られます。"
		if [ ! -e "${g_Kubectl_PATH}/kubectl.exe" ]; then
			EchoNextCommand_func

			az aks install-cli
		else
			echo  "Skipped. Because already installed."
		fi


		echo  ""
		ColorEcho_func  "クラスターと接続する。 = kubectl コマンドの対象となるクラスターを設定する\n"  "Green"
		echo  "\"${HOME}/.kube/config\" が作られます。"
		EchoNextCommand_func

		az aks get-credentials  --resource-group "${g_ResourceGroupName}"  --name "${g_ClusterName}"

		AddDependencies_func  "${g_AzureAKS_DependenciesTitle}"  "${g_AzureAKS_Dependencies[@]}"
	fi
}


#********************************************************************
# Function: Uninstall_AzureAKS_func
#********************************************************************
function  Uninstall_AzureAKS_func()
{
	if [ "$(az account list 2> /dev/null)" == "[]" ]; then
	...
	az group delete --name "${g_ResourceGroupName}"  --yes  --no-wait
	EchoNextCommand_func

	rm -rf  "${HOME}/.kube"
	EchoNextCommand_func

	rm -rf  "${g_Kubectl_PATH}"
	EchoNextCommand_func

	rm -rf  "${g_Kubelogin_PATH}"
	if [ "$(az account list 2> /dev/null)" != "[]" ]; then
		EchoNextCommand_func
	...
	RemoveDependencies_func  "${g_AzureAKS_DependenciesTitle}"
}


#********************************************************************
# Function: UninstallWithConfirm_AzureAKS_func
#********************************************************************
function  UninstallWithConfirm_AzureAKS_func()
{
	echo  ""
	ColorEcho_func  "Uninstall Azure AKS Quick Start application in the cluster, kubectl and kubelogin ...\n"  "Green"
	GetSharedDependencies_func  "${g_AzureAKS_DependenciesTitle}"  "${g_AzureAKS_Dependencies[@]}"
		#// g_SharedDependencies = .
	local  uninstalled="${False}"
	if [ -v g_SharedDependencies["AzureAKS"] ]; then
		echo  "Skipped. Because Azure AKS is used by other modules"
	fi
	Pause_func
	if [ ! -v g_SharedDependencies["AzureAKS"] ]; then

		Uninstall_AzureAKS_func
	fi
}
